...
class MapsGeneralService {
  ...
  Future<RouteMap> getRouteByCoordinates(
      LatLng start, LatLng destination) async {
    switch (_apiSelected) {
      case ApiMap.google:
        return await _googleMapsService.getRouteByCoordinates(
            start, destination);
      case ApiMap.mapBox:
  ...
  Future<List<Place>> getDestinationBySearch(
      String query, LatLng currentPosition) async {
    switch (_apiSelected) {
      case ApiMap.google:
        return await _googleMapsService.getDestinationBySearch(
            query, currentPosition);
      case ApiMap.mapBox:
        return await _mapBoxService.getDestinationBySearch(
            query, currentPosition);
      default:
        return await _googleMapsService.getDestinationBySearch(
            query, currentPosition);
  ...
  Future<void> updateCameraSpecificLocationZoom(
      LatLng location, double zoom, GoogleMapController mapController) async {
